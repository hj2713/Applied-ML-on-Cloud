...
- Strict class consistency checks between train and validation labels
...
## 10) Nsight roofline capture (optional but recommended)

Important:

- The main python benchmark command automatically captures metrics.csv, profiler traces, gpu_samples, and loss_history for all configurations in its run matrix.
- Nsight Compute (ncu) is external to the script. It does not automatically run for every model/config unless you launch it manually.

Create a dedicated folder for Nsight outputs:

mkdir -p outputs/nsight

Recommended capture on each GPU VM (T4 and A100), one file per model:

mkdir -p outputs/nsight
GPU_TAG=t4
ncu --set full --section SpeedOfLight_RooflineChart
--target-processes all
--csv --log-file outputs/nsight/${GPU_TAG}_resnet50_smoke_roofline.csv
python benchmark_roofline.py --model resnet50 --quick-check-real

On A100, set GPU_TAG=a100 before running the same loop.


sudo ncu --set full --section SpeedOfLight_RooflineChart --target-processes all --csv --log-file nsight_roofline_proof.csv ./venv/bin/python nsight_proof.py


If you want more Nsight granularity:

- Repeat Nsight runs with different model-only invocations or temporary constant overrides.
- Keep log-file names unique per run so outputs are never overwritten.

Notes:

- Nsight runs are slower than normal runs.
- Prefer representative captures (for example one per model per GPU type) unless you specifically need all combinations.
